...
- [x] 2026-03-15（已于 2026-03-12 提前完成）
  - [x] 完成最后一个功能项：最小版优雅关闭
  - [x] 范围控制在：`SIGINT/SIGTERM`、停止 accept、停止 worker、DB 安全析构
  - [x] 做完后冻结功能开发，正式进入文档期
  - [x] 固化一套 `redis-cli` / `nc` 联调命令，作为 README、复盘文档、演示脚本输入
- [ ] 文档期（自 2026-03-12 起）
...
- [ ] 时间线执行原则
  - [ ] 从 2026-03-12 起，优先级调整为：阶段复盘 > README / 文档主线 > 演示脚本 > 证据链记录
  - [ ] 每完成一个子阶段，立即回看本文件勾选状态，避免实现与 TODO 脱节
  - [ ] 自文档期开始，不再新增功能；若出现新问题，只有在“直接阻塞演示、文档结论或手动验证”时才允许继续改代码

## 收尾策略（从 2026-03-12 起生效）

- [x] `优雅关闭` 已作为最后一个功能项完成，当前进入文档期，不再主动扩展新能力
...
- [x] M2：完成 Phase 4 收尾（在当前可演示版本基础上，再补最小版优雅关闭）
