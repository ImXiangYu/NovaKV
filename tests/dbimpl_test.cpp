...
size_t CountNumericFilesWithExt(const std::string& dir,
...
TEST_F(DBImplTest, KeepLevelMappingAfterRestart) {
  ...
  const size_t recovered_total =
      db_recovered.LevelSize(0) + db_recovered.LevelSize(1);
...
TEST_F(DBImplTest, ManifestCheckpointTruncatesLogAtThreshold) {
  constexpr int kSeedWalFiles =
      98;  // 构造阶段会额外产生 2 条 edit，总计 100 次
  for (int i = 1; i <= kSeedWalFiles; ++i) {
    const std::string wal_path =
        test_db_path + "/" + std::to_string(i) + ".wal";
    std::ofstream ofs(wal_path, std::ios::binary | std::ios::trunc);
    ASSERT_TRUE(ofs.good()) << wal_path;
  }

  ASSERT_EQ(CountNumericFilesWithExt(test_db_path, ".wal"),
            static_cast<size_t>(kSeedWalFiles));
